/* ===== 手机增强插件 v1.2.0（房东模拟器适配） ===== */
    (function() {
        (function () {
            ...
            var phoneCtx = null, wallpapers = [], wpIndex = 0, currentPage = 0, dragState = null, wpPageEl = null, contentArea = null, clockTimer = null, navTimer = null, initCount = 0;
            function getPhoneRoot() {
                ...
                for (var d = 0; d < docs.length; d++) {
                    var doc = docs[d];
                    try { var hosts = doc.querySelectorAll('[id^="improved-phone-shadow-host-"]'); for (var i = 0; i < hosts.length; i++) { if (hosts[i].shadowRoot) return { doc: doc, root: hosts[i].shadowRoot }; } } catch (e) { }
                    try { var all = doc.querySelectorAll('*'); for (var j = 0; j < all.length; j++) { var sr = all[j].shadowRoot; if (sr && sr.querySelector && sr.querySelector('.phone-container')) return { doc: doc, root: sr }; } } catch (e) { }
                }
                return null;
            }
            ...
            function getVar(key, defaultVal) { try { var fn = getVarFn('getVariables'); if (fn) { var v = fn({ type: 'character' }); if (v && key in v) return v[key]; } } catch (e) { } return defaultVal; }
            function setVar(key, value) { try { var fn = getVarFn('insertOrAssignVariables'); if (fn) { var obj = {}; obj[key] = value; fn(obj, { type: 'character' }); return true; } } catch (e) { } return false; }
            function showToast(msg, isError) { try { if (typeof toastr !== 'undefined' && toastr) { isError ? toastr.error(msg) : toastr.success(msg); return; } } catch (e) { } console.log('[PhoneEnhanced]' + (isError ? ' [错误]' : ''), msg); }
                        ...
                        img.onload = function () {
                            try {
                                ...
                                if (file.type === 'image/png') {
                                    ...
                                    if (pngUrl.length > 2 * 1024 * 1024) { ctx2d.fillStyle = '#ffffff'; ctx2d.fillRect(0, 0, canvas.width, canvas.height); ctx2d.drawImage(img, 0, 0, canvas.width, canvas.height); resolve(canvas.toDataURL('image/jpeg', quality || 0.85)); } else { resolve(pngUrl); }
                                } else { ctx2d.drawImage(img, 0, 0, canvas.width, canvas.height); resolve(canvas.toDataURL('image/jpeg', quality || 0.85)); }
                            } catch (err) { resolve(originalDataUrl); }
                        };
            ...
            function setVueInputValue(input, value) {
                ...
                try { input.dispatchEvent(new win.Event('input', { bubbles: true })); input.dispatchEvent(new win.Event('change', { bubbles: true })); } catch (e) { input.dispatchEvent(new Event('input', { bubbles: true })); input.dispatchEvent(new Event('change', { bubbles: true })); }
            }
            function pickImageFile(multiple, callback) {
                ...
                input.addEventListener('change', function (e) { var files = e.target && e.target.files; if (!files || files.length === 0) return; try { callback(Array.prototype.slice.call(files)); } catch (err) { showToast('处理文件时出错', true); } });
                ...
                btn.addEventListener('click', function (e) {
                    ...
                    pickImageFile(false, function (files) {
                        ...
                        fileToDataUrl(files[0], maxDim || 512, 0.85).then(function (dataUrl) {
                            setVueInputValue(urlInput, dataUrl); showToast(successMsg || '图片已上传');
            ...
            function injectDialogUploadButtons() {
                ...
                for (var i = 0; i < urlInputs.length; i++) {
                    ...
                    bindUploadButton(uploadBtn, urlInput, 512, '图片已上传，请填写名称');
            ...
            function injectImagePickerUpload() {
                ...
                for (var i = 0; i < pickerModals.length; i++) {
                    ...
                    var uploadBtn = phoneCtx.doc.createElement('button'); uploadBtn.type = 'button'; uploadBtn.className = 'pe-upload-btn'; uploadBtn.style.width = '44px'; uploadBtn.style.flexShrink = '0'; uploadBtn.innerHTML = '<i class="fas fa-folder-open"></i>'; uploadBtn.title = '本地上传';
                ...
                for (var j = 0; j < urlModals.length; j++) {
                    ...
                    var uBtn = phoneCtx.doc.createElement('button'); uBtn.type = 'button'; uBtn.className = 'pe-upload-btn'; uBtn.innerHTML = '<i class="fas fa-folder-open"></i><span>本地上传图片</span>';
                    bindUploadButton(uBtn, uInput, 800, '图片已上传');
                    uBody.appendChild(uBtn);
                }
            }
            function loadWallpapers() { wallpapers = getVar(VAR_WALLPAPERS, []); if (!Array.isArray(wallpapers)) wallpapers = []; wpIndex = getVar(VAR_WALLPAPER_IDX, 0); if (typeof wpIndex !== 'number' || wpIndex < 0 || wpIndex >= wallpapers.length) wpIndex = 0; }
            function saveWallpapers() { setVar(VAR_WALLPAPERS, wallpapers); setVar(VAR_WALLPAPER_IDX, wpIndex); }
            function applyWallpaper() {
                if (!phoneCtx) return;
                var root = phoneCtx.root;
                var existing = root.getElementById('pe-wallpaper-style'); if (existing) existing.remove();
                ...
                var style = phoneCtx.doc.createElement('style'); style.id = 'pe-wallpaper-style';
            ...
            function updateWpDot() {
                if (!phoneCtx) return;
                var root = phoneCtx.root; var phoneContainer = root.querySelector('.phone-container'); if (!phoneContainer) return;
            ...
            function createWallpaperPage() {
                ...
                page.innerHTML = '<div class="pe-wp-top"><button class="pe-wp-btn pe-wp-manage-btn" title="管理壁纸"><i class="fas fa-images"></i></button></div><div class="pe-wp-center"><div class="pe-wp-clock">--:--</div><div class="pe-wp-date">--</div></div><div class="pe-wp-hint"><i class="fas fa-chevron-right" style="font-size:10px;"></i> 向右滑动返回</div>';
            ...
            function goToWallpaperPage() { if (!phoneCtx || !wpPageEl || !contentArea) return; currentPage = 1; contentArea.classList.add('pe-wp-sliding'); contentArea.style.transform = 'translateX(-100%)'; wpPageEl.classList.add('pe-wp-active'); wpPageEl.classList.remove('pe-wp-dragging'); wpPageEl.style.transform = 'translateX(0)'; updateWpClock(); updateWpDot(); }
            function goToHomePage() { if (!phoneCtx || !wpPageEl || !contentArea) return; currentPage = 0; contentArea.classList.add('pe-wp-sliding'); contentArea.style.transform = 'translateX(0)'; wpPageEl.classList.remove('pe-wp-active'); wpPageEl.classList.remove('pe-wp-dragging'); wpPageEl.style.transform = 'translateX(100%)'; updateWpDot(); }
            function openWallpaperManager() {
                ...
                function addWallpapersFromLocal() {
                    pickImageFile(true, function (files) {
                        ...
                        var promises = files.map(function (f) { return fileToDataUrl(f, 1080, 0.85).catch(function (err) { return null; }); });
                        Promise.all(promises).then(function (results) {
                            var added = 0;
                            for (var i = 0; i < results.length; i++) { if (results[i]) { wallpapers.push(results[i]); added++; } }
            ...
            function setupSwipe() {
                if (!phoneCtx) return;
                var phoneContainer = phoneCtx.root.querySelector('.phone-container'); if (!phoneContainer) return;
                ...
                var SWIPE_THRESHOLD = 50, MOVE_THRESHOLD = 8;
                ...
                function onMove(e) {
                    ...
                    var dx = dragState.currentX - dragState.startX, dy = dragState.currentY - dragState.startY;
            ...
            function injectTenantApp() {
                ...
                var sampleApp = root.querySelector('.app-block'); if (!sampleApp) return;
                var appContainer = sampleApp.parentNode; if (!appContainer) return;
                if (!root.getElementById('pe-tenant-app-style')) {
                    var style = phoneCtx.doc.createElement('style'); style.id = 'pe-tenant-app-style';
                    style.textContent = '.pe-tenant-app{display:flex;flex-direction:column;align-items:center;gap:4px;cursor:pointer;}.pe-tenant-app-icon{width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#667eea,#764ba2);display:flex;align-items:center;justify-content:center;color:#fff;font-size:20px;box-shadow:0 2px 8px rgba(102,126,234,0.35);}.pe-tenant-app-label{font-size:10px;color:#333;text-align:center;margin-top:2px;}.pe-tenant-app:active .pe-tenant-app-icon{transform:scale(0.92);}';
                    root.appendChild(style);
                }
                var app = phoneCtx.doc.createElement('div'); app.id = 'pe-tenant-app'; app.className = 'app-block pe-tenant-app';
                app.innerHTML = '<div class="pe-tenant-app-icon"><i class="fas fa-home"></i></div><span class="pe-tenant-app-label">住户管理</span>';
                app.addEventListener('click', function (e) {
                    e.preventDefault(); e.stopPropagation();
                    function findTM() {
                        var wins = [];
                        try { wins.push(window); } catch (e) {} try { if (window.parent) wins.push(window.parent); } catch (e) {} try { if (window.top) wins.push(window.top); } catch (e) {}
                        var names = ['openCM', 'cmOpen', 'openTenantManager', 'toggleTenantManager', 'showTenantManager', 'openCompanyManager', 'toggleCompanyManager'];
                        for (var i = 0; i < wins.length; i++) { for (var j = 0; j < names.length; j++) { try { if (typeof wins[i][names[j]] === 'function') return wins[i][names[j]]; } catch (e) {} } try { if (wins[i].tenantManager && typeof wins[i].tenantManager.open === 'function') return wins[i].tenantManager.open; } catch (e) {} try { if (wins[i].companyManager && typeof wins[i].companyManager.open === 'function') return wins[i].companyManager.open; } catch (e) {} }
                        return null;
                    }
                    var fn = findTM(); if (fn) { fn(); return; }
            ...
            function init() {
                try {
                    initCount++;
                    var newCtx = getPhoneRoot(); if (!newCtx) return;
                    var ctxChanged = (!phoneCtx || phoneCtx.root !== newCtx.root); phoneCtx = newCtx;
                    ...
                    injectDialogUploadButtons(); injectImagePickerUpload(); injectTenantApp();
